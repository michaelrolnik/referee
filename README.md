...
- **A fuller language server.** `referee-lsp` now provides live in-editor **diagnostics** (parse + type errors), **completion** (names in scope + keywords, narrowing to members after a `.`), **hover** (a name's declaration), **go-to-definition** (following `import`s across files), **document symbols** (the outline view), **find-references** (every use of a name, across imports), and **rename** (rewrite a name and all its uses, across imports), wired into the VS Code extension — see *Language server (LSP)*. A full IDE feature set is in place; further refinements (type-aware reference disambiguation, signature help) build on the same typed AST.
